...
- **Status:** Conditionally accepted for internal development; TM0-01 must be resolved before Phase 1 begins
- **Next review:** After Phase 0 remediation and before Part 1.1 authorisation
...
| ------ | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TM0-01 | Elevation / disclosure            | An authenticated caller invokes `app_private.is_active_platform_owner(user_id)` or `has_active_membership(user_id, business_id)` with arbitrary UUIDs, creating a platform-role or membership existence oracle outside the intended self-check. | Medium (2×2)                                                      | UUIDs are non-sequential; RLS policies pass `auth.uid()`; functions expose boolean only.                                                                 | **Open blocker before Phase 1.** Remove caller-supplied user identity or enforce equality with `auth.uid()`; add direct-RPC abuse tests and re-run RLS matrix. |
...
| TM0-04 | Elevation                         | Tenant or platform user bypasses RLS through grants, helper functions, user-editable claims, or service credentials.                                                                                                                            | High (2×3)                                                        | Revoked defaults, explicit grants, RLS, protected role tables, `SECURITY DEFINER` safe search path, isolation tests, no browser service-role client.     | Partially mitigated; TM0-01 must close and every new table/RPC requires matrix tests.                                                                          |
...
- **TM0-01:** Replace or constrain callable authorization helpers so an authenticated user cannot query another user's role/membership existence. Add tests that call each function directly using self and non-self identifiers and prove cross-user denial/non-disclosure.
...
The owner may continue internal development with synthetic data after reviewing this model. Phase 1 must not begin until TM0-01 is mitigated and verified. The current foundation is not approved for real-vendor demonstration, real data, staging pilot, or production.
